...
(comment defn metadata? [string]
  (not (nil? (re-seq #"\w+ *: *[\w, ]+" (str string)))))

(comment defn extract-metadata
...
(comment defn slash-str "Returned string will start with a leading slash."
   [string]
  (if (= \/ (nth string 0 ""))
    string
    (str "/" string) ))

(comment defn make-vec "Create a vector from a comma delimited string"
  [string]
  (vec (filter
        #(not (= 0 (count %1))) ; drop blanks
        (split #"[\s,]" (str string)) ))) ; seed a str

(comment defrecord Article [title uri-path section author tags description content])

(comment defn make-article "Return a functional Article given the free-form text"
   [text]
